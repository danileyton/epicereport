...
function local_epicereports_render_sidebar(string $active = '', ?stdClass $course = null): void {
    ...
    if ($course && !empty($course->id)) {
        ...
        // Separador y opciones de reportes programados
        $items[] = [
            'id'    => 'separator1',
        ...
        $items[] = [
            'id'    => 'schedules',
            'label' => get_string('schedules', 'local_epicereports'),
        ...
        $items[] = [
            'id'    => 'separator2',
            'label' => get_string('tools', 'local_epicereports'),
            'url'   => null,
            'icon'  => '',
            'separator' => true,
        ];
        ...
        $items[] = [
            'id'    => 'test_reports',
            'label' => get_string('testreports', 'local_epicereports'),
        ...
        $items[] = [
            'id'    => 'test_email',
            'label' => get_string('testemail', 'local_epicereports'),
            'url'   => new moodle_url('/local/epicereports/test_email.php', ['courseid' => $course->id]),
            'icon'  => 'fa-envelope',
        ];
    }
    ...
    foreach ($items as $item) {
        ...
        if (!empty($item['separator'])) {
            echo html_writer::start_tag('li', ['class' => 'epice-nav-separator']);
            echo html_writer::tag('span', $item['label'], ['class' => 'epice-nav-separator-text']);
